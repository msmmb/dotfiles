...
def fondo(bg):
    if bg in [f'0{i}' if i < 10 else str(i) for i in range(1,len(os.listdir('/home/michael/Imágenes/Fondos de Pantalla'))+1)]:
        nuevo = 'feh --bg-fill /home/michael/Imágenes/Fondos\\ de\\ Pantalla/{}.jpg\n'.format(bg)
        os.system(nuevo)
        escribir("/home/michael/.config/bspwm/bspwmrc", [nuevo], "feh", [0])
...
def opacidad(op):
        if 0 <= float(op) and float(op) <= 1:
            escribir("/home/michael/.config/alacritty/alacritty.yml", ['  opacity: {}\n'.format(op)], "  opacity:",[0])
...
def tamaño_fuente(tm):
    if float(tm) > 6:
        escribir("/home/michael/.config/alacritty/alacritty.yml", ['  size: {}\n'.format(tm)], "size", [0])
    else:
        print("Fuente demasiado pequeña")

def padding(pd):
    escribir("/home/michael/.config/alacritty/alacritty.yml", ['    x: {}\n'.format(pd), '    y: {}\n'.format(pd)], "padding", [1,2])
...
def fuente(ft):
    ...
    if ft in fuentes:
        escribir("/home/michael/.config/alacritty/alacritty.yml", [f'    family: {fuentes.get(ft)}\n' for i in range(3)], "family", [0,3,6])
    else:
...
def sep(sp):
    ...
    if int(sp) in range(1,5):
        escribir("/home/michael/.config/polybar/modules.ini", [f"content = \"{separadores.get(int(sp))}\"\n"], "content", [0])
    else:
...
def tema(tm):
    ...
    if tm in temas:
        ...
        if tm == "basic" or tm == "xmonad":
            escribir("/home/michael/.config/polybar/modules.ini", [f"content = \"\"\n"], "content", [0])
